from src.gpts import gpt_client
from src.gpts.gpt_client import GPTClient


class ErrorGPTClient(GPTClient):
    ...
    def __init__(self, model_name="gpt-4o", task_description=None):
        #read the system prompt from prompts/error_gpt_system.txt and # replace <TASK_DESCRIPTION_HERE> with the actual task description
...
if __name__ == "__main__":
    # Example usage
    error_gpt = ErrorGPTClient(task_description="The task is to place a cube on the circular target position.")
    observation_id = error_gpt.upload_image("data/test/cube_observation.png")
    goal_id = error_gpt.upload_image("data/test/cube_goal.png")
    ...
    print(f"Error Category: {error_category}, Response ID: {response_id}")
